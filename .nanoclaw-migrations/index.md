...
> **Phase 2 APPLIED on `.12` — 2026-08-17.** `main` was reset onto the upstream
> base and the customizations reapplied. Pre-migration state is preserved at tag
> `pre-migrate-2ca297ce-20260817-173621` (rollback: `git reset --hard` that tag).
> Deviations from the plan below, all validated (build + 1590 host tests + 311
> agent-runner tests + container typecheck all green):
> - **§5 context_mode DROPPED** — upstream rewrote scheduling to `ncl tasks` with
>   isolated task sessions by default; legacy recurring tasks keep firing
>   unchanged (see `docs/ncl-tasks-migration.md`), so no data migration is needed.
> - **Gmail/Cal** installed via `container/cli-tools.json` (upstream refactored the
>   Dockerfile global-install into that manifest; the skills' Dockerfile prose was
>   stale). `zod-to-json-schema@3.22.5` co-listed to keep the gmail pin. No
>   `TOOL_ALLOWLIST` edit — claude.ts derives `mcp__*` patterns dynamically.
> - **telegram-markdown-sanitize** dropped — absent from the `channels` branch;
>   upstream telegram.ts renders MarkdownV2 natively.
> - **Docs §4** — kept upstream README/CLAUDE.md as the accurate base and folded
>   fork customizations in, instead of restoring 877-commits-stale fork docs.
> - **`.100` rollout still pending.** New pre-rollout gate: stamp the upgrade
>   marker before restarting the service — `pnpm exec tsx scripts/upgrade-state.ts
>   set` (CHANGELOG [BREAKING]; host refuses to boot otherwise).
